...
Текущая проверка 15.09: **103/103 tests в npm test прошли**, включая 10 новых outcome-тестов и обновлённые 11 commitment-тестов. Компиляция Solidity прошла. Gas sweep: 12 локальных settlement transactions, N=10..1000, K=10; максимальное наблюдаемое total 7 363 734 gas. [Алгоритм, результат и пределы эксперимента](SHORT_OUTCOME_VERIFICATION.md). Предыдущие пакеты: commitment 93/93, lifecycle 82/82. Настоящий PAIR/router на local fork проверен предыдущим этапом [direct BUY](DIRECT_BUY_REPLAY.md); новый сетевой fork на этапе outcome не запускался.
...
USDG funding/monthly accounting, регистрация, basket math, BUY/attempt replay, атомарный Short commitment и deterministic outcome реализованы отдельными компонентами. Target для MVP deployment — 100 USDG. Следующая граница — ограниченная публичная активация версий с защитой OPEN attempts/carry, авторизация snapshot, readiness/расписание/выбор D, cutoff/finality и аутентификация единственного seed. Денежный settlement + расход attempts проверены только в тестовом outcome controller; production terminal ещё нужен. Один immutable controller должен поддерживать также Monthly до deployment. Численные настройки, предел N/calldata/gas и конвертация остаются открыты. Lifecycle replay отдельно не проверяет outcome; новый verifier пересчитывает его по заданному seed, но не доказывает честность/доставку seed. Публичный запуск не готов.
