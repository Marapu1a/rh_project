...
Обновлено по локальному коду и проверкам: 19.09.2026. Это карта кода, а не утверждение реализации всей [продуктовой схемы](../../PRODUCT_SPEC.md).

Текущий этап 19.09: [BUY → два Short-цикла](../../LOCAL_BUY_CYCLE.md).
Локальные торговые транзакции → существующий HTTP scanner → replay v4 → builder →
публикация/verify → freeze/settlement → повторный replay → следующий Short → claim.
Проверяются carry, поздние BUY, сохранение Monthly и откат terminal. Торговый стенд
и RNG тестовые, это не PAIR fork и не production indexer/keeper.
Соседние проверки вместе с новым сценарием: **18/18 passed**. Основной список теперь
176; полный объединённый прогон 176 не выполнялся, последний полный — 175/175 ниже.

Предыдущий этап 19.09: [локальный скелет контроллеров](../../LOCAL_CONTROLLER_SKELETON.md).
LocalShortController и LocalMonthlyController соединены с общей казной и отдельным
тестовым async RNG. Проверены полный Short до claim и следующего цикла, одновременный
Monthly, recovery другим исполнителем и атомарный отказ request. Только chainId 31337;
участники синтетические, BUY replay ещё не соединён с этим сценарием. Новые тесты 3/3.
Runtime Short 22 237, Monthly 17 453, vault 8 496 bytes. Production binding отсутствует.
Полный `npm test` после добавления скелета: **175/175 passed**, 19.09.2026 (~507 s).

Предыдущий этап 18.09: [drand binding model](../../DRAND_BINDING_MODEL.md), JavaScript-only.
...
[первичный отбор RNG](../studies/RNG_PROVIDER_STUDY_2026-09-17.md). Monthly suite **13/13 passed**;
...
| Исследование полного controller | 17.09 отдельный исполняемый макет RNG/roles/readiness/Monthly, шесть compile profiles. Обычный root 28 475 байт; viaIR+fixed helper+runs=1 — 24 444, лишь 132 байта запаса. Production contracts/settings не менялись. [Измерения и ограничения](../studies/CONTROLLER_SIZE_STUDY.md) |
| Оптимизация Short selection | 17.09 общий validated primitive, повторное использование ranks; runtime fixture 18 497 байт вместо 20 340. Пять сравнений газа на одинаковом state/seed с exact resultHash и vault accounting. Новых внешних helpers нет. [Результаты](../studies/SHORT_SELECTION_OPTIMIZATION.md) |
