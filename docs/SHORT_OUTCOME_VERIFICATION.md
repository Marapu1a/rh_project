...
Все 12 транзакций поместились в локальный блок. Total — receipt gas всей fixture settlement transaction, включая calldata и тестовые события. Verification/finalize — разницы gasleft внутри fixture, не самостоятельные transaction gas; refunds/обвязка означают, что нельзя просто приравнять сумму внутренних замеров к receipt gas. Компонент finalize включает O(K²) проверку уникальности vault, K здесь равен 10.
