...
> Ответ получен (`6dab1fa`, подтверждение владельца `f421083`), запрос закрыт. Следующий принятый пакет реализован: [direct BUY evidence и replay v1](DIRECT_BUY_REPLAY.md). Ниже исходный запрос для связи с ответом; вопросы про payer/nominal threshold/узкий route повторно не открываем.
