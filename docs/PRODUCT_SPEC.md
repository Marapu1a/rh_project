...
Обновлено: 12.09.2026, после согласования внешнего funding и переполнения Next Start.
...
Предел Next учитывается внутри пополнения, даже если оно пересекает цель. Дроби в raw units и независимость итоговых долей от дробления поступлений — требования реализации, не повод использовать приближённые 33,33%/16,67%. Кандидат для неизбежных остатков округления — Current; окончательный алгоритм должен сохранять всю сумму и не создавать её повторно.
...
Прямой ERC20 transfer не содержит выбранного назначения. Способ распознавания таких пополнений и их классификацию как общего funding необходимо определить в API; нельзя приписать отправителю несуществующую инструкцию. Поддержка внешнего TOKEN funding с сохранением назначения через последующую конвертацию также ещё не специфицирована. Первый этап — USDG.
...
Первый определённый по смыслу этап — USDG funding и три призовых резерва: общее/целевое пополнение, предел Next и overflow в Current, отсутствие project fee с внешних денег и невозможность двойного учёта.

До реализации нужно зафиксировать API и технические детали: распознавание прямых переводов, точное округление, авторизацию перехода jackpot, учёт pending/current additions и связь с уже существующим PromoVault. Это не повод считать существующие три recipients FeeRouter тремя новыми призовыми резервами.
