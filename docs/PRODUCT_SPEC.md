...
**Принято 24.09:** обмен может запускаться по предварительно достаточному пополнению
Short с учётом GENERAL. Бюджет и призы опираются на фактический USDG после swap;
при недостаче накопление продолжается. Точный курс/сумма не обещаются, допустимое
отклонение ограничено защитой сделки. На сайте показывается фактическая транзакция.
Временные и прочие gates сохраняются. [Подробности](CONVERSION_TRIGGER.md).
