...
Направление активации: публичный payload версии → заранее объявленный момент применения → привязка правил к участию/draw. Точные границы параметров, полномочия объявления, срок предупреждения и переход уже накопленных OPEN attempts/carry ещё требуют дизайна. «Менять только до freeze» недостаточно: нельзя задним числом переписать условия накопленного участия. Текущий внутренний API хранит payload каждого draw, но не предоставляет production-admin или обход этих требований через произвольный выбор правил исполнителем.
