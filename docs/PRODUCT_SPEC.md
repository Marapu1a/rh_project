...
15.09, технический этап: [атомарный Short commitment](SHORT_DRAW_COMMITMENT.md) реализован как внутренняя часть будущего полного immutable controller. Он фиксирует заявленный snapshot и корзину вместе с USDG reserve, но не доказывает правильность билетов и не реализует полный draw. Production числа и продуктовая математика этим этапом не изменены.
