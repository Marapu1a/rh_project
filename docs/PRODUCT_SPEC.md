...
Обновлено: 14.09.2026, после согласования Short Luck и корзины short-призов. Продуктовая логика обновлена; реализация остаётся на этапе USDG funding/monthly accounting. Формулы, параметры draws и creator shares этим не утверждены.
...
1. Раз в 6 часов проверяем готовность бюджета и билетов. Не готово → пропуск без расходования попыток и изменения Luck.
2. До random фиксируем бюджет D из свободного Short, обеспеченную корзину ненулевых USDG-призов, участников, entries, usedLuck и версию правил. Сумма корзины не выше D. Как именно строится корзина из бюджета, ещё требуется выбрать.
...
Это заменяет прежний кандидат «сгенерировать каждому сумму и проверить остаток», а также старую схему SpendFactor + нормализованные веса. Порог ненулевого осмысленного приза и состав корзины пока не выбраны. Не требуется заранее обеспечить приз каждому потенциально допущенному кошельку.
...
## 9. Шансы и состояние Luck: что ещё не выбрано

**Принято:** cap итогового шанса кошелька в одном draw, отсутствие гарантированного победителя и запрет reroll ради удобного результата. Для short шанс получения денег не выше шанса допуска: приз можно получить только после допуска. Рост Luck при изменившейся конкуренции не гарантирует рост итогового шанса относительно прошлого draw.

Кандидат Codex:

```text
q(e,L) = p_max × (e + L) / (e + L + h), если e > 0
q(0,L) = 0
```

Один Luck здесь эквивалентен одному дополнительному entry только для расчёта допуска; эта эквивалентность и формула **ещё не утверждены**. Альтернатива GPT: q_base=p_max×e/(e+h), q=p_max−(p_max−q_base)×r^L, также с обязательным e>0. Параметры, предел хранения Luck и точное округление вероятности не выбраны.

Не подписываем вероятность допуска как точный шанс получить USDG. При фиксированных K и составе остальных допущенных честная случайная раздача снижает этот шанс при дефиците призов. Cap одного draw не ограничивает шанс за множество draws или совокупность нескольких кошельков.

Минимальное состояние-кандидат: persistent shortLuck кошелька, неизменяемые entries/usedLuck/rulesVersion в draw snapshot и однократное обновление по окончательно назначенным призам. История результата должна позволять проверить состояние; место хранения и проверка participant snapshot ещё требуют дизайна indexer/controller.

Предложение для concurrency — один pending short на всю систему, новые попытки копятся для следующего, checkpoints не образуют очередь. Следующий freeze только после завершения предыдущего draw и применения обновлений Luck. **Это ещё не принятое правило:** задержка RNG тогда останавливает новые short, хотя funding, claims и monthly могут продолжаться независимо.
...
Предлагается: no-win возвращает frozen jackpot в Current и сохраняет Next; win назначает frozen сумму в claimable и переносит Next в Current, сохраняя новые поступления после freeze. Все участвовавшие месячные попытки расходуются при состоявшемся random; при недоставленном результате остаются pending, не объявляются проигравшими. Точный порядок перехода и sync перед ним вынесен в текущее обращение к GPT. 100 USDG старта не означает изменение отдельного правила $100 BUY за entry.
