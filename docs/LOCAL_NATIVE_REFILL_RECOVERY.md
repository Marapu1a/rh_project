...
- Исходный пакет проверял продолжение после нового запуска. С 23.09 CLI --watch повторяет
  распознанные временные read outages и polling известного pendingReceipt: см.
  [watch policy](LOCAL_PROMO_COORDINATOR.md#watch-временная-недоступность-rpc-23092026).
  Одноразовый CLI и неизвестные/опасные ошибки остаются fail-closed; supervisor не добавлен.
