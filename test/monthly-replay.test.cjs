...
function fixture(){
  ...
  const announce=()=>emit('MonthlyRulesAnnounced',[2,outcome.rulesHash(nextRules),time()+11]);
  const activate=()=>emit('MonthlyRulesActivated',[1,2,h.head().blockNumber+2]);
  ...
  const empty=()=>{const cutoff=h.head(),snapshot=emptyMonthlyEpochHash(domainFor(h.manifest,h.config),1,cutoff,outcome.rulesHash(normalRules));
    emit('MonthlyEpochEmpty',[1,cutoff.blockNumber,cutoff.blockHash,snapshot]);};
