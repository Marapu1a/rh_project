...
test('trigger uses GENERAL Short share, not total sale proceeds',()=>{
  assert.equal(plan({...base,expectedUSDG:100n}).reason,'accumulating');
  assert.equal(plan(base).shouldAttempt,true);
  assert.equal(plan({...base,freeShort:100n}).reason,'alreadyFunded');
});
...
test('forecast is not actual funding: a smaller receipt may leave reserve below target',()=>{
  ...
  assert.equal(plan({...base,freeShort:99n,expectedUSDG:0n}).reason,'accumulating');
});
