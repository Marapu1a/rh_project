...
async function main(){
  ...
  const provider=new ethers.JsonRpcProvider(request,undefined,{cacheTimeout:-1}),stop=new AbortController();
  const interrupt=()=>stop.abort();process.once('SIGINT',interrupt);process.once('SIGTERM',interrupt);
  try{
    ...
    async function pass(){
      if(!options){
        ...
        options={statePath:o.state,signal:stop.signal,ops,nativeRefill,
    ...
    process.exitCode=await runWatch({pass,watch:!!o.watch,signal:stop.signal,
      pollMs:(ops?ops.settings.pollSeconds:job.pollSeconds)*1000,emit:r=>console.log(JSON.stringify(r))});
