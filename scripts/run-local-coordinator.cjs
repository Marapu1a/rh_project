...
async function main(){
  ...
  for(let i=0;i<args.length;i++){
    ...
    if(!['--job','--config','--state','--scheduler-state','--rpc','--publisher','--executor'].includes(args[i])||args[i+1]==null)
      throw Error('Expected --job FILE --config FILE --state FILE --scheduler-state FILE --rpc LOOPBACK [--publisher INDEX] [--executor INDEX] [--watch]');
  ...
  try{
    ...
    const options={statePath:o.state,signal:stop.signal,
    ...
    do{
      ...
      await new Promise(resolve=>{
        ...
        const timer=setTimeout(done,job.pollSeconds*1000);stop.signal.addEventListener('abort',done,{once:true});if(stop.signal.aborted)done();
      });
