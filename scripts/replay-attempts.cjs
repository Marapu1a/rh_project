...
async function main(){
  ...
  const result={inputMode,ledger,ledgerHash:hash(ledger)};
  if(options['--verify']){
  ...
  console.log(JSON.stringify({inputMode,ledgerHash:result.ledgerHash,pending:ledger.pending,wallets:ledger.wallets,draws:ledger.draws.map(d=>({drawId:d.drawId,kind:d.kind,status:d.status,totalAttempts:d.totalAttempts}))},null,2));
}
