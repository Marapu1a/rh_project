...
async function main(){
 ...
 for(let i=0;i<args.length;i+=2){if(!['--state','--expected','--rpc'].includes(args[i])||args[i+1]==null)throw Error('Use --state FILE --expected FILE --rpc LOOPBACK');o[args[i].slice(2)]=args[i+1];}
